...
BEGIN_MESSAGE_MAP(CAssignmentDlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_BN_CLICKED(IDC_BUTTON_PURCHASE, &CAssignmentDlg::OnBnClickedButtonPurchase)
	ON_CBN_SELCHANGE(IDC_COMBO_MOVE, &CAssignmentDlg::OnCbnSelchangeComboMove)
END_MESSAGE_MAP()
...
void CAssignmentDlg::OnCbnSelchangeComboMove()
{
	...
	case 0:
		if (leftIndex != LB_ERR)
		{
			m_listLeft.GetText(leftIndex, str);
			m_listLeft.DeleteString(leftIndex);
			m_listLeft.SetCurSel(leftIndex - 1);
	...
	case 1:
		...
		{
			m_listRight.GetText(rightIndex, str);
			m_listRight.DeleteString(rightIndex);
			m_listRight.SetCurSel(rightIndex - 1);
...
void CAssignmentDlg::OnBnClickedButtonPurchase()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	CString str;

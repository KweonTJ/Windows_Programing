...
{
	CDialogEx::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_TREE_CONTTROL, m_treeControl);
	DDX_Text(pDX, IDC_EDIT_SELECTED_NODE, m_strSelectedNode);
	DDX_Text(pDX, IDC_EDIT_NAME_TEXT, m_strNodeText);
}
...
BEGIN_MESSAGE_MAP(CExample2Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_NOTIFY(TVN_SELCHANGED, IDC_TREE_CONTTROL, &CExample2Dlg::OnSelchangedTreeConttrol)
	ON_BN_CLICKED(IDC_BUTTON_INSERT, &CExample2Dlg::OnClickedButtonInsert)
	ON_BN_CLICKED(IDC_BUTTON_MODIFY, &CExample2Dlg::OnClickedButtonModify)
	ON_BN_CLICKED(IDC_BUTTON_DELETE, &CExample2Dlg::OnClickedButtonDelete)
	ON_BN_CLICKED(IDC_CHECK_EXPAND, &CExample2Dlg::OnClickedCheckExpand)
END_MESSAGE_MAP()
...
void CExample2Dlg::OnSelchangedTreeConttrol(NMHDR* pNMHDR, LRESULT* pResult)
{
...
void CExample2Dlg::OnClickedButtonInsert()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
...
void CExample2Dlg::OnClickedButtonModify()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	UpdateData(TRUE);
	if(!m_strNodeText.IsEmpty())
	{
		...
		{
			AfxMessageBox(_T("루트 노드는 수정할 수 없습니다."));
		}
	}
	else
	{
...
void CExample2Dlg::OnClickedButtonDelete()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.
	if(m_hSelectedNode != m_hRoot)
	{
...
void CExample2Dlg::OnClickedCheckExpand()
{
	// TODO: 여기에 컨트롤 알림 처리기 코드를 추가합니다.

...
BEGIN_MESSAGE_MAP(CExample1Dlg, CDialogEx)
	...
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB_SELECTION, &CExample1Dlg::OnSelchangeTabSelection)
END_MESSAGE_MAP()
...
BOOL CExample1Dlg::OnInitDialog()
{
	...
	m_tabSelection.InsertItem(3, _T("애니메이션"));
	...
	m_dlgAnimation.Create(IDD_DIALOG_ANIMATION, &m_tabSelection);
	m_dlgAnimation.GetWindowRect(&rect);
	m_dlgAnimation.MoveWindow(5, 25, rect.Width(), rect.Height());
...
void CExample1Dlg::UpdateDrawing()
{
	// TODO: 여기에 구현 코드 추가.
	CRect rectView;
	GetDlgItem(IDC_STATIC_VIEW)->GetWindowRect(&rectView);
...
void CExample1Dlg::OnSelchangeTabSelection(NMHDR* pNMHDR, LRESULT* pResult)
{
